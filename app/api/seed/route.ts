...
const SEED_DATA = [
  {
    ...
    actionItems: [
      { title: 'Add user data deletion endpoint at /api/user/delete', description: 'Implement a REST endpoint that accepts authenticated deletion requests and initiates the cascade deletion workflow for all user data.', priority: 'HIGH' as const, techStackRelevance: ['web', 'api', 'database'] },
      { title: 'Implement cascade deletion for user-generated content', description: 'Ensure all user-generated content including posts, comments, uploads, and metadata are deleted when a user exercises their right to erasure.', priority: 'HIGH' as const, techStackRelevance: ['web', 'database', 'storage'] },
      { title: 'Add data export functionality (DSAR compliance)', description: 'Build a data subject access request export feature that generates a downloadable archive of all user data in a portable format.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'api', 'storage'] },
      { title: 'Update privacy policy page with erasure request process', description: 'Document the erasure request process, expected timelines, and user rights clearly on the privacy policy page.', priority: 'LOW' as const, techStackRelevance: ['web', 'legal'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Implement automated breach detection monitoring', description: 'Set up real-time monitoring for unauthorized data access patterns and anomalous database queries that could indicate a breach.', priority: 'HIGH' as const, techStackRelevance: ['aws', 'gcp', 'azure', 'security'] },
      { title: 'Create incident response notification system', description: 'Build an automated notification pipeline that alerts the security team and triggers compliance workflows within minutes of breach detection.', priority: 'HIGH' as const, techStackRelevance: ['web', 'api', 'email'] },
      { title: 'Document and test incident response playbook', description: 'Create a comprehensive incident response playbook and schedule quarterly tabletop exercises to validate the process.', priority: 'MEDIUM' as const, techStackRelevance: ['security', 'operations'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Implement Global Privacy Control (GPC) signal detection', description: 'Detect the Sec-GPC header in incoming requests and automatically apply opt-out preferences for data sale and sharing.', priority: 'HIGH' as const, techStackRelevance: ['web', 'api', 'privacy'] },
      { title: 'Update consent management platform for GPC compliance', description: 'Modify the cookie consent banner to auto-apply opt-out settings when a GPC signal is present without prompting the user.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'frontend', 'privacy'] },
      { title: 'Add "Do Not Sell My Info" link to footer', description: 'Ensure a visible Do Not Sell or Share My Personal Information link is present in the website footer on all pages.', priority: 'LOW' as const, techStackRelevance: ['web', 'frontend'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Add privacy nutrition label to App Store Connect', description: 'Complete the privacy nutrition label questionnaire in App Store Connect with accurate data collection and usage declarations.', priority: 'HIGH' as const, techStackRelevance: ['ios', 'app-store'] },
      { title: 'Implement ATT (App Tracking Transparency) prompt', description: 'Add the App Tracking Transparency framework prompt to request user permission before accessing the IDFA for tracking purposes.', priority: 'HIGH' as const, techStackRelevance: ['ios', 'app-store', 'mobile'] },
      { title: 'Remove third-party SDKs without Apple privacy manifests', description: 'Audit all third-party SDKs and remove or replace any that do not include the required PrivacyInfo.xcprivacy manifest file.', priority: 'MEDIUM' as const, techStackRelevance: ['ios', 'app-store', 'mobile'] },
      { title: 'Create privacy manifest for first-party code', description: 'Generate a PrivacyInfo.xcprivacy file documenting all required reason APIs used by your app and the justification for each.', priority: 'MEDIUM' as const, techStackRelevance: ['ios', 'app-store'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Evaluate StoreKit External Purchase API implementation', description: 'Review the new StoreKit External Purchase API and determine if implementing external payment links is beneficial for EU users of the app.', priority: 'MEDIUM' as const, techStackRelevance: ['ios', 'app-store', 'payments'] },
      { title: 'Implement Apple-mandated disclosure sheet for external links', description: 'If using external payments, implement the required disclosure sheet that Apple mandates before redirecting users to external purchase flows.', priority: 'LOW' as const, techStackRelevance: ['ios', 'app-store', 'payments'] },
      { title: 'Set up external transaction reporting to Apple', description: 'Build a reporting mechanism to send transaction data to Apple for purchases completed through external payment links as required.', priority: 'LOW' as const, techStackRelevance: ['ios', 'app-store', 'payments', 'api'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Audit data safety section accuracy in Play Console', description: 'Review and update the data safety section in Google Play Console to accurately reflect all data types collected and shared by the app.', priority: 'HIGH' as const, techStackRelevance: ['android', 'play-store'] },
      { title: 'Implement in-app account and data deletion', description: 'Provide a functional account deletion option within the app that removes all user data as declared in the data safety section.', priority: 'HIGH' as const, techStackRelevance: ['android', 'play-store', 'api'] },
      { title: 'Document all third-party SDK data practices', description: 'Create an inventory of all third-party SDKs, their data collection practices, and ensure each is properly declared in the data safety section.', priority: 'MEDIUM' as const, techStackRelevance: ['android', 'play-store', 'mobile'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Update targetSdkVersion to API level 34', description: 'Update the app build configuration to target Android 14 (API level 34) and resolve all deprecation warnings and breaking changes.', priority: 'HIGH' as const, techStackRelevance: ['android', 'play-store', 'mobile'] },
      { title: 'Declare foreground service types', description: 'Add the required foreground service type declarations in the manifest for all foreground services used by the app.', priority: 'HIGH' as const, techStackRelevance: ['android', 'mobile'] },
      { title: 'Migrate to new photo/video permissions model', description: 'Update media access code to use the new granular photo and video permissions introduced in API 34 instead of READ_EXTERNAL_STORAGE.', priority: 'MEDIUM' as const, techStackRelevance: ['android', 'mobile'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Implement payment page script integrity monitoring', description: 'Deploy a Content Security Policy and script integrity monitoring solution for all payment pages to detect unauthorized script modifications.', priority: 'HIGH' as const, techStackRelevance: ['web', 'payments', 'security'] },
      { title: 'Enable MFA for all administrative access', description: 'Implement multi-factor authentication for all admin panel access, server SSH, and payment system dashboards as required by v4.0.', priority: 'HIGH' as const, techStackRelevance: ['web', 'api', 'security', 'aws'] },
      { title: 'Set up automated security log review', description: 'Deploy automated log analysis tools that review security logs daily and alert on anomalous patterns related to payment data access.', priority: 'MEDIUM' as const, techStackRelevance: ['aws', 'gcp', 'security', 'operations'] },
      { title: 'Deploy Content Security Policy headers on payment pages', description: 'Configure strict CSP headers on all pages that handle payment card data to prevent injection of unauthorized scripts.', priority: 'HIGH' as const, techStackRelevance: ['web', 'payments', 'security'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Update password policy to 12-character minimum', description: 'Modify user registration and password change flows to enforce a minimum 12-character password length with complexity requirements.', priority: 'HIGH' as const, techStackRelevance: ['web', 'api', 'payments', 'security'] },
      { title: 'Implement account lockout after 10 failed attempts', description: 'Add rate limiting and account lockout logic that locks accounts for 30 minutes after 10 consecutive failed login attempts.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'api', 'security'] },
      { title: 'Configure 15-minute session idle timeout', description: 'Set session idle timeout to 15 minutes for all interfaces that access payment card data or cardholder data environment components.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'api', 'payments'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Implement end-to-end encryption for video sessions', description: 'Ensure all telehealth video and audio streams use end-to-end encryption that prevents even the platform operator from accessing session content.', priority: 'HIGH' as const, techStackRelevance: ['web', 'mobile', 'healthcare', 'webrtc'] },
      { title: 'Build secure virtual waiting room feature', description: 'Create a virtual waiting room that keeps patients in a private holding state until the provider is ready to begin the consultation.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'healthcare', 'frontend'] },
      { title: 'Verify BAA coverage for all telehealth vendors', description: 'Audit all technology vendors in the telehealth delivery chain and ensure current Business Associate Agreements are signed and on file.', priority: 'MEDIUM' as const, techStackRelevance: ['healthcare', 'legal', 'operations'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Enable encryption at rest for all ePHI databases', description: 'Configure AES-256 encryption at rest for all databases and storage systems containing electronic Protected Health Information.', priority: 'HIGH' as const, techStackRelevance: ['aws', 'gcp', 'azure', 'database', 'healthcare'] },
      { title: 'Schedule annual third-party penetration test', description: 'Engage a qualified third-party security firm to conduct annual penetration testing of all systems in the ePHI environment.', priority: 'HIGH' as const, techStackRelevance: ['security', 'healthcare', 'operations'] },
      { title: 'Implement quarterly phishing simulation program', description: 'Deploy a phishing simulation platform to conduct quarterly exercises and track workforce security awareness improvement over time.', priority: 'MEDIUM' as const, techStackRelevance: ['security', 'operations', 'healthcare'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Run WCAG 2.2 AA audit on all public-facing pages', description: 'Conduct a comprehensive accessibility audit using automated tools and manual testing against all new WCAG 2.2 Level AA success criteria.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'frontend', 'accessibility'] },
      { title: 'Implement focus appearance indicators (WCAG 2.4.11)', description: 'Ensure all interactive elements have a visible focus indicator that meets the new minimum area and contrast requirements in WCAG 2.2.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'frontend', 'css', 'accessibility'] },
      { title: 'Add alternatives to dragging interactions (WCAG 2.5.7)', description: 'Provide non-dragging alternatives for all drag-and-drop interactions so users who cannot perform dragging movements can still complete all tasks.', priority: 'LOW' as const, techStackRelevance: ['web', 'frontend', 'accessibility'] },
      { title: 'Implement consistent help mechanism (WCAG 3.2.6)', description: 'Ensure help mechanisms like contact information, chat widgets, or FAQ links appear in a consistent location across all pages.', priority: 'LOW' as const, techStackRelevance: ['web', 'frontend', 'accessibility'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Map existing controls to updated SOC 2 TSC criteria', description: 'Review the updated Trust Service Criteria and map your existing security controls to identify gaps that need to be addressed before the next audit.', priority: 'MEDIUM' as const, techStackRelevance: ['security', 'operations', 'compliance'] },
      { title: 'Implement software supply chain security controls', description: 'Deploy dependency scanning, SBOM generation, and artifact signing to address new supply chain integrity requirements in the updated criteria.', priority: 'MEDIUM' as const, techStackRelevance: ['web', 'api', 'security', 'devops'] },
      { title: 'Document API security controls for audit evidence', description: 'Create documentation of API authentication, authorization, rate limiting, and input validation controls for SOC 2 audit evidence collection.', priority: 'LOW' as const, techStackRelevance: ['api', 'security', 'compliance'] },
    ],
  },
  {
    ...
    actionItems: [
      { title: 'Classify AI systems by EU AI Act risk tier', description: 'Assess all AI and ML systems to determine their risk classification under the EU AI Act framework and identify which systems fall under high-risk requirements.', priority: 'MEDIUM' as const, techStackRelevance: ['ai', 'ml', 'compliance'] },
      { title: 'Implement AI system logging and documentation', description: 'Create technical documentation and implement comprehensive logging for high-risk AI systems including training data, model decisions, and performance metrics.', priority: 'MEDIUM' as const, techStackRelevance: ['ai', 'ml', 'api', 'database'] },
      { title: 'Build human oversight interface for high-risk AI decisions', description: 'Develop an interface that allows human reviewers to monitor, understand, and override AI system decisions for systems classified as high-risk.', priority: 'LOW' as const, techStackRelevance: ['ai', 'ml', 'web', 'frontend'] },
    ],
  },
];
...
export async function GET(request: Request) {
  ...
  if (expectedSecret && secret !== expectedSecret) {
    return NextResponse.json(
      { error: "Unauthorized. Provide ?secret=YOUR_NEXTAUTH_SECRET" },
      { status: 401 }
    );
  }

  // Check if database already has regulations (idempotent)
  const existingCount = await prisma.regulation.count();
  if (existingCount > 0) {
    return NextResponse.json({
      message: "Database already seeded",
      regulationCount: existingCount,
    });
  }

  // Seed the database
  let created = 0;
  for (const reg of SEED_DATA) {
